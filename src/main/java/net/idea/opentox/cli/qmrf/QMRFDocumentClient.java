...
public class QMRFDocumentClient<POLICY_RULE> extends
		AbstractURIClient<QMRFDocument, POLICY_RULE> {
...
{"qmrf": [
{
	 ...
	 */
	@Override
	protected List<QMRFDocument> processPayload(InputStream in, String mediaType)
		...
		if (mime_json.equals(mediaType)) {
			...
			if (data != null)
				for (int i = 0; i < data.size(); i++) {
					...
					try {
						document
								.setTitle(docNode.get("title").getTextValue());
					} catch (Exception x) {
					}
					if (list==null) list = new ArrayList<QMRFDocument>();
					list.add(document);
				}
			return list;
			/*
