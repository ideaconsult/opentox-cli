 ...
 * @param <POLICY_RULE>
 */
public class CompoundClient<POLICY_RULE> extends
		AbstractURIClient<Compound, POLICY_RULE> {
	...
	public List<URL> searchURI(URL url, String term, boolean b64)
		...
		if (b64)
			return listURI(
					url,
					new String[] { b64search_param,
							Base64.encodeBase64String(term.getBytes()) });
		else
		...
		if (mime_json.equals(mediaType)) {
			...
			if (data != null)
				for (int i = 0; i < data.size(); i++) {
					...
					try {
						substance
								.setInChI(compound.get("inchi").getTextValue());
					} catch (Exception x) {
					}
					try {
						substance.setInChIKey(compound.get("inchikey")
								.getTextValue());
					} catch (Exception x) {
					}
					try {
						substance.setSMILES(compound.get("smiles")
								.getTextValue());
					} catch (Exception x) {
					}
					try {
						substance.setFormula(compound.get("formula")
								.getTextValue());
					} catch (Exception x) {
					}
					if (list == null)
					...
					while (fields.hasNext()) {
						...
						String type = features.get(field.getKey())
								.get("sameAs").getTextValue();
						if ("http://www.opentox.org/api/1.1#ChemicalName"
								.equals(type)) {
							if (!"".equals(field.getValue().getTextValue()))
								substance.setName(field.getValue()
										.getTextValue());
						} else if ("http://www.opentox.org/api/1.1#IUPACName"
								.equals(type)) {
							if (!"".equals(field.getValue().getTextValue()))
								substance.setIupacName(field.getValue()
										.getTextValue());
						} else if ("http://www.opentox.org/api/1.1#SMILES"
								.equals(type)) {
							substance
									.setSMILES(field.getValue().getTextValue());
						} else if ("http://www.opentox.org/api/1.1#CASRN"
								.equals(type)) {
							substance.setCas(field.getValue().getTextValue());
						} else if ("http://www.opentox.org/api/1.1#EINECS"
								.equals(type)) {
							substance
									.setEinecs(field.getValue().getTextValue());
						} else if ("http://www.opentox.org/api/1.1#InChI_std"
								.equals(type)) {
							substance.setInChI(field.getValue().getTextValue());
						} else if ("http://www.opentox.org/api/1.1#InChIKey_std"
								.equals(type)) {
							substance.setInChIKey(field.getValue()
									.getTextValue());
						} else if ("http://www.opentox.org/api/1.1#REACHRegistrationDate"
								.equals(type)) {
							//
						} else if (Compound.opentox_ChEBI.equals(type)) {
							substance.getProperties().put(type,
									field.getValue().getTextValue());
							...
							substance.getProperties().put(
									key,
									(value.getTextValue() == null ? Double
											.toString(value.getDoubleValue())
											: value.getTextValue()));
						}
					}
				}
	...
	public RemoteTask setSubstancePropertyAsync(URL serviceRoot,
			Compound substance, String customidName, String customidValue)
			throws InvalidInputException, Exception {
	...
	protected HttpEntity createFormEntity(Compound substance,
			String customidName, String customidValue)
			throws UnsupportedEncodingException {
		List<NameValuePair> formparams = new ArrayList<NameValuePair>();
		if (substance.getResourceIdentifier() != null)
			formparams.add(new BasicNameValuePair("compound_uri", substance
					.getResourceIdentifier().toExternalForm()));
		// formparams.add(new BasicNameValuePair("molfile", ??));
		if (substance.getCas() != null)
			formparams.add(new BasicNameValuePair(
					Compound._titles.CASRN.name(), substance.getCas()));
		if (substance.getEinecs() != null)
			formparams.add(new BasicNameValuePair(Compound._titles.EINECS
					.name(), substance.getEinecs()));
		if (substance.getName() != null)
			formparams.add(new BasicNameValuePair(Compound._titles.ChemicalName
					.name(), substance.getName()));
		if (substance.getInChI() != null)
			formparams.add(new BasicNameValuePair(Compound._titles.InChI_std
					.name(), substance.getInChI()));
		if (substance.getInChIKey() != null)
			formparams.add(new BasicNameValuePair(Compound._titles.InChIKey_std
					.name(), substance.getInChIKey()));
		if (substance.getIUCLID_UUID() != null)
			formparams.add(new BasicNameValuePair(Compound._titles.IUCLID5_UUID
					.name(), substance.getIUCLID_UUID()));
		if ((customidName != null) && (customidValue != null)) {
			formparams
					.add(new BasicNameValuePair("customidname", customidName));
	...
	public List<Compound> get(URL url, String mediaType, String... params)
		...
		LOGGER.log(
				Level.INFO,
				"See API-DOCS at http://ideaconsult.github.io/examples-ambit/apidocs/#!/compound");
